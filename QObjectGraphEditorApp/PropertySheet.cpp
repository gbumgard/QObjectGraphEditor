...
int PropertySheetPrivate::enumToInt(const QMetaEnum &metaEnum, int enumValue) const
{
...
int PropertySheetPrivate::intToEnum(const QMetaEnum &metaEnum, int intValue) const
{
...
bool PropertySheetPrivate::isSubValue(int value, int subValue) const
{
  if (value == subValue)
    return true;
  int i = 0;
...
bool PropertySheetPrivate::isPowerOf2(int value) const
{
  while (value) {
    if (value & 1) {
      return value == 1;
...
int PropertySheetPrivate::flagToInt(const QMetaEnum &metaEnum, int flagValue) const
{
  if (!flagValue)
    return 0;
  int intValue = 0;
...
int PropertySheetPrivate::intToFlag(const QMetaEnum &metaEnum, int intValue) const
{
  QMap<int, bool> valueMap; // dont show multiple enum values which have the same values
  QList<int> values;
  for (int i = 0; i < metaEnum.keyCount(); i++) {
...
void PropertySheetPrivate::updateClassProperties(const QMetaObject *metaObject, bool recursive)
{
  if (!metaObject)
    return;
...
void PropertySheetPrivate::addClassProperties(const QMetaObject *metaObject)
{
  if (!metaObject)
    return;

  //addClassProperties(metaObject->superClass());
  QVariant propVal = m_object->property("methodOffset");
  if (propVal.isValid()) {
    int methodOffset = propVal.toInt();
    if (methodOffset) {
      if (methodOffset < metaObject->methodOffset()) {
        addClassProperties(metaObject->superClass());
      }
    }
  }
...
void PropertySheetPrivate::saveExpandedState()
{

}

void PropertySheetPrivate::restoreExpandedState()
{

}

void PropertySheetPrivate::slotValueChanged(QtProperty *property, const QVariant &newValue)
{
  ...
  if (metaProperty.isEnumType()) {
    if (metaProperty.isFlagType())
      metaProperty.write(m_object, intToFlag(metaProperty.enumerator(), newValue.toInt()));
    else
      metaProperty.write(m_object, intToEnum(metaProperty.enumerator(), newValue.toInt()));
  }
  else {
    metaProperty.write(m_object, newValue);
  }

  emit q_ptr->slotValueChanged(property, oldValue, newValue);
...
PropertySheet::PropertySheet(QWidget *parent)
  : QWidget(parent)
{
  d_ptr = new PropertySheetPrivate;
  d_ptr->q_ptr = this;
...
void PropertySheet::setObject(QObject *object)
{
...
QObject *PropertySheet::object() const
{
  return d_ptr->m_object;
}

#include "moc_PropertySheet.cpp"
